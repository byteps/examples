...
def get_train_op_and_metrics(loss, params):
  ...
  with tf.variable_scope("get_train_op"):
    ...
    if params["distribution_strategy"] == "horovod":
      import horovod.tensorflow as hvd
      optimizer = hvd.DistributedOptimizer(optimizer)

    if params["distribution_strategy"] == "byteps":
...
def construct_estimator(flags_obj, params, schedule_manager):
  ...
  """
  if not params["use_tpu"]:
    if params["distribution_strategy"] in ['horovod', 'byteps']:
      if params["distribution_strategy"] == 'horovod':
        import horovod.tensorflow as ds
      else:
        import byteps.tensorflow as ds

      config = tf.ConfigProto()
      config.gpu_options.allow_growth = True
      config.gpu_options.visible_device_list = str(ds.local_rank())
...
def run_transformer(flags_obj):
  ...
  params["distribution_strategy"] = flags_obj.distribution_strategy
  ...
  if params["distribution_strategy"] in ['horovod', 'byteps']:
    ...
    ds.init()
    train_hooks += [ds.BroadcastGlobalVariablesHook(0)]
