...
class UGATIT(object) :
    ...
    def train(self):
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            if bps.local_rank() == 0:
                this_time = time.time()
                print("[%5d/%5d] time: %4.4f speed: %4.4f (sec/step) d_loss: %.8f, g_loss: %.8f"  % (step, self.iteration, 
                            this_time - start_time, this_time - last_time, Discriminator_loss, Generator_loss))
                last_time = this_time
